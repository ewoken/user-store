...
describe('user service', () => {
  describe('signUp(newUser, user)', () => {
    ...
    test('should fail for a bad user', async () => {
      ...
      await expect(userService.signUp(badUser, user))
        .rejects
        .toThrow(/password/)
    })
    ...
    test('should fail for an existing email', async () => {
      ...
      await expect(userService.signUp(newUser, user))
        .rejects
        .toThrow(/plop@plop.com/)
    })
    ...
    test('should fail if already logged', async () => {
      ...
      await expect(userService.signUp(newUser, newUser))
        .rejects
        .toThrow(/logged/)
    })
  })
  ...
  describe('logIn(credentials, user)', () => {
    ...
    beforeEach(() => {
      return userService.signUp(testUser, user)
    })
    ...
    test('should fail with bad email', async () => {
      ...
      await expect(userService.logIn(credentials, user))
        .rejects
    ...
    test('should fail if logged', async () => {
      await expect(userService.logIn(testUser, testUser))
        .rejects
        .toThrow(/logged/)
    })
  })
